...
dictioanry
'''
for root, dirs, song_files in os.walk("./Midi_Files/AmbroseTabsDatabase/ClassicalMusic"):
    for filename in song_files:
        ...
        while(filename[len(filename)-offset]!='('):
            ...
            offset+=1
        num = int(num)
        filename = "./Midi_Files/AmbroseTabsDatabase/ClassicalMusic/"+str(filename)
        files[filename] = num
...
for file in files:
    ...
    for chord in sOnlyChords:
        ...
        if chord_name not in unique_chords and len(chord)>2:
            #in case chords has commas in name
            unique_chords.append(chord_name)

        #put chord in chords_in_piece dictionary
        if(len(chord) > 2):
            if chord_name not in chords_in_piece:
                chords_in_piece[chord_name] = 1
            else:
                chords_in_piece[chord_name] += 1
...
f = open("music_IrisNCAmbroseClassical.csv","w")
labels_file = open("music_IrisNCAmbroseClassical.txt","w")
f.write("length,key,number_of_notes,total_measures,"
...
#     if i+1<len(unique_chords):
#         labels_file.write(",")
#     i+=1


# print("total chords: ",len(unique_chords))
f.write("Grade\n")
...
for id,feature_list in songs_features.items():
    ...
    f.write(str(songs_features[id][0])+","+str(songs_features[id][1])
    ...
    #         song_chord_index+=1
    #         count+=1
    # print("chord count: ",count)
    #write grade for piece
    grade = str(feature_list[len(feature_list)-1]);
    f.write(grade)
    print(grade)
    f.write("\n")
