...
def averageRangeForNextNotes(chords):
    ...
    if chords is not None:
        while i<len(chords)-1:
            ...
            i+=1
        if(len(chords)>0):
            average = float(total)/float(len(chords))
...
def totalNotes(given_stream):
    if(given_stream.getElementsByClass("Note") is not None):
        return float(len(given_stream.getElementsByClass("Note")))
    else:
        return float(0)
    ...
    if given_stream is not None:
        ...
        for note in notes:
            if '#' in note.name:
                totalSharpNotes+=1
                #print(note.name)
    return float(totalSharpNotes)*float(2)
    ...
    if given_stream is not None:
        ...
        for note in notes:
            if '-' in note.name:
                totalFlatNotes+=1
                #print(note.name)
    return float(totalFlatNotes)*float(2)
...
def averageAccidentals(given_stream,altered_pitches):
    ...
    if given_stream is not None:
        sChords = given_stream.chordify()
        sFlat = sChords.flat
        notes = sFlat.getElementsByClass('Chord')
...
songs_features = dict()
feature_index = 0
for root, dirs, song_files in os.walk("MusicXML_Files/PianoMarvelSongs/RegularSongs/"):#"MusicXML_Files/PianoMarvelSongs/RegularSongs/"):
    for file in song_files:
        feature_values = dict()
        print("file: ",file)

        curr_stream = converter.parse("MusicXML_Files/PianoMarvelSongs/RegularSongs/"+str(file))#"MusicXML_Files/PianoMarvelSongs/RegularSongs/"+str(file))
    ...
    ####################################################################extract custom features
        ...
        try:
            feature_values['MostCommonNoteQuarterLength'] = float(features.native.MostCommonNoteQuarterLength(score).extract().vector[0])
        except:
        ...
        try:
            feature_values['MostCommonNoteQuarterLengthPrevalence'] = float(features.native.MostCommonNoteQuarterLengthPrevalence(score).extract().vector[0])
        except:
