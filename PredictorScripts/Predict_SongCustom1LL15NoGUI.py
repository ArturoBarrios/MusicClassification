...
def averageLargeJumps(chords,largeJump):
    ...
    if chords is not None:
        while i<len(chords)-1:
            ...
            if(abs(int(re.search(r'\d+', str(aInterval.name)).group()))>=largeJump):
                totalLargeJumps+=1
...
songs_features = dict()
feature_index = 0
for root, dirs, song_files in os.walk("MusicXML_Files/PianoMarvelSongs/RegularSongs/"):
    for file in song_files:
        feature_values = dict()
        print("file: ",file)

        curr_stream = converter.parse("MusicXML_Files/PianoMarvelSongs/RegularSongs/"+str(file))
...
# performance_unknown = []
# count = 0
# #apply logarithmic function to each value(add 1 every value in case value is 0)
# for feature,value in unknown_feature_values[unknown_song_grade].items():
#     if count<len(unknown_feature_values[unknown_song_grade])-1 and count<500:
#         value = round(float(value),3)
#         value = value+2
#         value = math.log(value)
#         objects_unknown.append(feature)
#         performance_unknown.append(value)
#     count+=1
#
# count = 0
...
# performance = []
# count = 0
# for feature,value in feature_values[unknown_song_grade].items():
#     if count<len(unknown_feature_values[unknown_song_grade])-1 and count<500:
#         value = round(float(value),3)
#         value = value+2
#         value = math.log(value)
...
# labels_dict = dict()
# labels_dict2 = dict()
# men_means = [20, 34, 30, 35, 27,60,60,34,34,54]
# women_means = [25, 32, 34, 20,25,23,43,54,54,54]
#
# x = np.arange(len(objects))  # the label locations
# width = .7  # the width of the bars
#
# #fig, ax = plt.subplots()
# fig, ax = plt.subplots(num=None, figsize=(206, 10), dpi=80, facecolor='w', edgecolor='k')
# ## I changed the fig size to something obviously big to make sure it worked
#
# rects1 = ax.bar(x - (width/4), performance, width/2, label='Known')
# rects2 = ax.bar(x + (width/4) , performance_unknown, width/2, label='Unknown')
# count1 = 0
# count2 = 0
# for rect in rects1:
#     x_pos = rect.get_x()+rect.get_width()/2.
#     labels_dict[str(x_pos)] = objects[count1]
#     count1+=1
#
# for rect in rects2:
#     x_pos = rect.get_x()+rect.get_width()/2.
#     labels_dict2[str(x_pos)] = objects[count2]
#     count2+=1
#
#
# # Add some text for labels, title and custom x-axis tick labels, etc.
# ax.set_ylabel('Feature Values')
# title_string = 'Feature Values For Unknown Song And Grade ', unknown_song_grade," songs"
...
#
# def update_annot(bar,labels):
#     x = bar.get_x()+bar.get_width()/2.
#     y = bar.get_y()+bar.get_height()
#     annot.xy = (x,y)
#     text = labels[str(x)],"  ",feature_values[unknown_song_grade][labels[str(x)]]
#     annot.set_text(text)
