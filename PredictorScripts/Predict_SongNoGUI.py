...
model_file = open("music_IrisWCUknown.txt","r")
...
for root, dirs, song_files in os.walk("./Unknown_Songs"):
    for file in song_files:

        print("file: ",file)
        chords_in_piece = dict()
        curr_stream = converter.parse("./Unknown_Songs/"+str(file))
...
f = open("music_IrisWCUknown.csv","w")
labels_file = open("music_IrisWCUknown.txt","w")
...
x_labels = []
f = open("music_IrisWCG3.txt","r")
line = f.read()
#put labels in x_labels
for label in line.split(","):
    x_labels.append(label)

X, y, type2id = loader.load_data('music_IrisWCUknown.csv', y_label="Grade", x_labels=x_labels)
